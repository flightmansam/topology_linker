...
def run_seepage(period_end):
    ...
    seepage_df = seepage(pools, period_start, period_end, days=days)
    seepage_df.to_csv(f"seepage_{period_start.strftime('%d-%m-%y')}.csv", index=True)

#export out to mi sql
period_start = pd.datetime(year=2020, month=8, day=11, hour=12)
period_end = pd.datetime(year=2020, month=10, day=20, hour=12)

dates = pd.date_range(period_start, period_end, freq='3D')
